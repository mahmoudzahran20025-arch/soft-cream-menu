/**
 * js/icons-loader.js
 * * يقوم هذا السكربت بتحميل الأيقونات الثانوية (غير الحرجة) بشكل مؤجل
 * لتسريع التحميل الأولي للصفحة (Initial Load).
 * يتم التحميل عند أول تفاعل من المستخدم (scroll, click, touch) 
 * أو عند فراغ المتصفح (requestIdleCallback).
 */

const secondaryIconsHTML = `
<svg xmlns="http://www.w3.org/2000/svg" style="display: none;" aria-hidden="true" id="secondary-icons-sprite">
  <symbol id="utensils" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  <symbol id="instagram" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  <symbol id="store" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  <symbol id="truck" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  <symbol id="shopping-basket" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    <path d="m15 11-1 9"/>
    <path d="m19 11-4-7"/>
    <path d="M2 11h20"/>
  ...
  <symbol id="calculator" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  <symbol id="shield-check" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10"/>
    <path d="m9 12 2 2 4-4"/>
  </symbol>
  <symbol id="chef-hat" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  <symbol id="package-search" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
/**
 * يقوم بإدراج الأيقونات الثانوية في نهاية الـ body.
 */
export function loadSecondaryIcons() {
  if (iconsLoaded || document.getElementById('secondary-icons-sprite')) {
    return; // تم التحميل مسبقاً
  }
  
  iconsLoaded = true;
  document.body.insertAdjacentHTML('beforeend', secondaryIconsHTML);
  
  // تنظيف الـ event listeners
  interactionEvents.forEach(event => {
    document.removeEventListener(event, lazyLoadIcons);
  });
}

/**
 * دالة التحميل الكسول التي يتم استدعاؤها عند التفاعل.
 */
...
/**
 * الاستماع لأول تفاعل من المستخدم لتحميل الأيقونات.
 * نستخدم { once: true, passive: true } لأفضل أداء.
 */
interactionEvents.forEach(event => {
  document.addEventListener(event, lazyLoadIcons, { once: true, passive: true });
});

/**
 * كـ Fallback، نقوم بتحميل الأيقونات بعد فترة قصيرة
 * إذا كان المتصفح في وضع الخمول (idle).
 */
if ('requestIdleCallback' in window) {
  requestIdleCallback(loadSecondaryIcons, { timeout: 3000 });
} else {
  // fallback لمتصفحات أقدم
  setTimeout(loadSecondaryIcons, 3000);
}